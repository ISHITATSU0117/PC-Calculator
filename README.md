...
- **共有結果**: 全ユーザーが同じ計算結果を表示（GitHubに保存）
...
1. **手動計算**
   ...
   - 計算結果はGitHubリポジトリに `calculation_results.json` として保存されます
...
**全ユーザーが同じ結果を表示**
- 管理画面で計算を実行すると、結果がGitHubリポジトリに保存されます
- 結果公開ページを開いたすべてのユーザーは、同じ保存された結果を表示します
- 新しいCSVファイルがアップロードされた場合は、管理画面で再計算を実行してください

**メリット**
- 全員が確実に同じデータを確認できます
- 計算済みの結果を読み込むため、表示が高速です
- ネットワークやタイミングの違いによる結果の不一致がありません
...
- **結果の共有**: 計算結果は `calculation_results.json` としてリポジトリに保存
- **認証**: Personal Access Token（アップロード・削除・計算に必要）
...
- **影響**: 結果公開ページは保存された結果ファイルを取得（1リクエスト/表示）
- **自動更新**: 不要（保存された結果を表示）

### 認証済みリクエスト（PAT使用）
- **制限**: トークンごとに1時間5000リクエスト
- **影響**: アップロード・削除・計算・結果保存の操作
- **推奨**: 複数の管理者で同じトークンを共有可能

### 推奨事項
- 結果公開ページは誰でも閲覧可能（保存された結果を表示）
- 計算は管理者が管理画面から実行（自動計算も可）
- 管理操作（アップロード・削除・計算）は5000リクエスト/時間まで可能
